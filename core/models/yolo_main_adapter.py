"""YOLO adapter for tray-level object detection (cup/plate/bowl)."""
...
from config.constants import MAIN_YOLO_CLASSES
from config.settings import MAIN_YOLO_CONF, MAIN_YOLO_IOU
from core.domain.entities import CropCandidate, DetectionBox
from core.domain.enums import DishDetectionSource


class YoloMainAdapter:
    """Runs YOLO_main on a full tray image and returns normalized domain entities."""
    ...
    def __init__(
        ...
        # Support both `weight_path` and legacy `weights_path` for ModelRegistry compatibility.
        resolved_path = weight_path or weights_path
        if resolved_path is None:
            raise ValueError("YOLO main weight path is required.")

        self.weight_path = Path(resolved_path)
        if not self.weight_path.exists():
            raise FileNotFoundError(f"YOLO main weights not found: '{self.weight_path}'")

        self.device = device
        self.conf = float(conf if conf is not None else MAIN_YOLO_CONF)
        self.iou = float(iou if iou is not None else MAIN_YOLO_IOU)
        self._allowed_classes = set(MAIN_YOLO_CLASSES)

        self._model: Any | None = None
        self._load_model()

    def _load_model(self) -> None:
        """Lazy import + model load with explicit dependency error message."""
        try:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load YOLO main model from '{self.weight_path}': {exc}") from exc

    def _ensure_ready(self) -> Any:
        if self._model is None:
            raise RuntimeError("YOLO main model is not loaded.")
        return self._model

    def _normalize_input_image(self, image: Any) -> Any:
        """Normalize ndarray/PIL/path-like input to a value accepted by ultralytics."""
    ...
    def _run_inference(self, image: Any) -> list[Any]:
        """Run predict and always return list-like results for robust postprocessing."""
    ...
    def _result_to_detection_boxes(self, results: list[Any]) -> list[DetectionBox]:
        """Map ultralytics Results to DetectionBox and keep only tray classes."""
    ...
    def _result_to_crop_candidates(self, detections: list[DetectionBox]) -> list[CropCandidate]:
        # We only build crop metadata; no disk I/O is performed inside adapters.
        return [
            CropCandidate(
                crop_id=f"yolo-main-{uuid4().hex}",
                bbox=box,
    ...
    # Warmup performs a tiny inference to initialize kernels/caches before real frames.
