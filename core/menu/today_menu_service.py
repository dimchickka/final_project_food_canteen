...
class TodayMenuService:
    ...
    def add_dish_to_today(self, category: DishCategory | str, dish_slug: str) -> Path:
        ...
        # Today menu is intentionally lightweight: we store a link to global dish card
        # and avoid copying heavy embeddings or images.
        link_payload = {
        ...
        today_meta = {
            ...
            "crop_image_path": global_meta.get("crop_image_path"),
            "crop_embedding_path": global_meta.get("crop_embedding_path"),
            "qwen_phrase_path": global_meta.get("qwen_phrase_path"),
            "qwen_phrase_embedding_path": global_meta.get("qwen_phrase_embedding_path"),
    ...
    def list_today_dishes(self, category: DishCategory | str | None = None) -> list[MenuDish]:
        ...
        for category_enum in categories:
            ...
            for dish_dir in sorted(entry for entry in category_dir.iterdir() if entry.is_dir()):
                link_path = dish_dir / "link.json"
                if not link_path.exists():
                    continue
                link_data = self._load_json(link_path)
                global_dish_dir = Path(str(link_data.get("global_dish_dir", "")))
                if not global_dish_dir.exists():
                    continue

                meta = self._load_meta(global_dish_dir)
    ...
    def rebuild_today_indexes(self) -> dict[str, Path]:
        ...
        # Builder scans dish directories in today root and creates category indexes.
