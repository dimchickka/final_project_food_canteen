...
class MenuRepository:
    ...
    def create_dish(
        ...
        if self.index_builder is not None:
            # After crop save we rebuild single-dish embeddings to keep card files fresh.
            ...
            # Keep category indexes reasonably up-to-date for retrieval consumers.
            self._rebuild_category_indexes(category_enum)
        ...
        *,
        ...
        if name is not None and name.strip():
            ...
            if rename_folder:
                ...
                # After folder rename every file path in meta must be recomputed,
                # otherwise card metadata points to stale pre-rename locations.
                meta["slug"] = new_slug
                self._refresh_meta_paths(dish_dir=dish_dir, meta=meta)
    ...
    # Best-effort category rebuild keeps indexes fresh while tolerating partially-filled text embeddings.
    def _rebuild_category_indexes(self, category: DishCategory) -> None:
        if self.index_builder is None:
            return

        category_dir = self._resolve_category_dir(category)
        indexes_root = category_dir / "indexes"

        try:
            self.index_builder.build_category_photo_index(
                category_dir=category_dir,
                output_index_dir=indexes_root / "photo",
            )
        except RuntimeError:
            pass

        try:
            self.index_builder.build_category_text_index(
                category_dir=category_dir,
                output_index_dir=indexes_root / "text",
            )
        except RuntimeError:
            pass

    def _refresh_meta_paths(self, dish_dir: Path, meta: dict[str, Any]) -> None:
        meta["folder_path"] = str(dish_dir)
        meta["crop_image_path"] = str(dish_dir / "crop.jpg")
        meta["crop_embedding_path"] = str(dish_dir / "crop_embedding.pt")
